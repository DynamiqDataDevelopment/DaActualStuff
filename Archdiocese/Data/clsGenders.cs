...
public class clsGenders_List : List<clsGenders_Item>
{
    private string _connectionString = string.Empty;

    public clsGenders_List(string connectionString, ref Exception pEx)
    {
        ...
        try
        {
            ...
            cmd.CommandText = "usp_GetGenders";
    ...
    private void Populate_Members(SqlDataReader data_reader)
    {
        ...
        if (data_reader.HasRows)
        {
            ...
            {
                clsGenders_Item tmp = new clsGenders_Item();
    ...
    public bool Add_Item(ref Exception pEx, int ID, string description)
    {
        ...
        try
        {
            ...
            SqlCommand cmd = conn.CreateCommand();
            cmd.CommandType = CommandType.StoredProcedure;
            cmd.CommandText = "usp_InsertGenders";
            cmd.Parameters.AddWithValue("@ID", ID);
            cmd.Parameters.AddWithValue("@description", description);
            if (Save(ref pEx, cmd))
            {
                this.Add(new clsGenders_Item(ID, description));
                conn.Close();
                return true;
            }
            else
    ...
    public bool Update_Item(ref Exception pEx, int ID, string description)
    {
        ...
        try
        {
            ...
            SqlCommand cmd = conn.CreateCommand();
            cmd.CommandType = CommandType.StoredProcedure;
            cmd.CommandText = "usp_UpdateGenders";
            cmd.Parameters.AddWithValue("@ID", ID);
            cmd.Parameters.AddWithValue("@description", description);
            if (Save(ref pEx, cmd))
            {
                foreach (clsGenders_Item Item in this)
                {
                    if (Item.ID == ID)
                    {
                        Item.ID = ID;
    ...
    public bool Delete_Item(ref Exception pEx, int ID)
    {
        ...
        try
        {
            ...
            SqlCommand cmd = conn.CreateCommand();
            cmd.CommandType = CommandType.StoredProcedure;
            cmd.CommandText = "usp_DeleteGenders";
            cmd.Parameters.AddWithValue("@ID", ID);
            if (Save(ref pEx, cmd))
            {
                conn.Close();
...
public class clsGenders_Item
{
    ...
    public clsGenders_Item()
    {
        //Default constructor
    }

    public clsGenders_Item(int ID, string description)
    {
        _ID = ID;
